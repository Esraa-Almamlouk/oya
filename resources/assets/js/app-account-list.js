...
'use strict';
    ...
    $(document).on('click', '.edit-record', function () {
        ...
        $('#add-account-balance').val($btn.data('balance') ?? 0);
    ...
    FormValidation.formValidation(addNewAccountForm, {
        fields: {
        name: { validators: { notEmpty: { message: ' اسم الحساب مطلوب' } } },
        category: { validators: { notEmpty: { message: 'اخنر تصنيف الحساب' } } },
        currency: { validators: { notEmpty: { message: 'اختر عملة الحساب' } } },
        bank: { validators: { notEmpty: { message: 'المصرف مطلوب' } } },
        balance: { validators: { numeric: { message: 'قيمة الحساب يجب ان تتكون من الارقام فقظ' } } },
        phone: {
          validators: {
            notEmpty: { message: 'رقم الهاتف مطلوب' },
            stringLength: {
              min: 10,
              max: 10,
